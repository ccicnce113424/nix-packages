...
echo "Pixiv Viewer git deps build output is:"
echo "$build_output"
hash_pixiv_viewer_git=$(tr -s ' ' <<<$build_output | grep -Po "got: \K.+$")
if [ -z "$hash_pixiv_viewer_git" ]; then
  echo "Failed to extract hash from build output."
  exit 1
fi
echo "Pixiv Viewer git deps hash is: $hash_pixiv_viewer_git"

jq -n \
  --arg version "$version" \
  --arg hash "$hash_pixiv_viewer_git" \
